...
import {IKarmaHookV2PoolExtension} from "./interfaces/IKarmaHookV2PoolExtension.sol";
import {IKarmaPoolExtensionAllowlist} from "./interfaces/IKarmaPoolExtensionAllowlist.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

import {IKarmaHook} from "../interfaces/IKarmaHook.sol";
import {IKarmaHookV2} from "./interfaces/IKarmaHookV2.sol";
...
abstract contract KarmaHookV2 is BaseHook, IKarmaHookV2 {
    ...
    IKarmaPoolExtensionAllowlist public immutable poolExtensionAllowlist;
    address public immutable weth;

    mapping(PoolId => bool) public karmaIsToken0;
    ...
    constructor(
        ...
        factory = _factory;
        poolExtensionAllowlist = IKarmaPoolExtensionAllowlist(_poolExtensionAllowlist);
        weth = _weth;
    }
    ...
    function _initializePoolExtensionData(
        ...
        if (_poolExtension != address(0)) {
            ...
            IKarmaHookV2PoolExtension(_poolExtension).initializePreLockerSetup(
                poolKey, karmaIsToken0[poolKey.toId()], poolExtensionData
            );
    ...
    function initializePool(
        ...
        // initialize the pool
        PoolKey memory poolKey =
            _initializePool(karma, pairedToken, tickIfToken0IsKarma, tickSpacing, poolData);
        ...
        emit PoolCreatedFactory({
            ...
            karma: karma,
            poolId: poolKey.toId(),
            tickIfToken0IsKarma: tickIfToken0IsKarma,
    ...
    function initializePoolOpen(
        ...
        // if able, we prefer that weth is not the karma as our hook fee will only
        // collect fees on the paired token
        if (karma == weth) {
            revert WethCannotBeKarma();
        }
        ...
        emit PoolCreatedOpen(
            pairedToken, karma, poolKey.toId(), tickIfToken0IsKarma, tickSpacing
        );
    ...
    function _initializePool(
        ...
        if (pairedToken == address(0) || karma == address(0)) {
            revert ETHPoolNotAllowed();
        }

        // determine if karma is token0
        bool token0IsKarma = karma < pairedToken;

        // create the pool key
        PoolKey memory _poolKey = PoolKey({
            currency0: Currency.wrap(token0IsKarma ? karma : pairedToken),
            currency1: Currency.wrap(token0IsKarma ? pairedToken : karma),
        ...
        karmaIsToken0[_poolKey.toId()] = token0IsKarma;

        // initialize the pool
        int24 startingTick = token0IsKarma ? tickIfToken0IsKarma : -tickIfToken0IsKarma;
        ...
        onlyFactory
    {
        ...
        IKarmaMevModule(mevModule[poolKey.toId()]).initialize(poolKey, mevModuleData);

        // give pool extension, if it exists, chance to check other configured settings
        if (poolExtension[poolKey.toId()] != address(0)) {
            IKarmaHookV2PoolExtension(poolExtension[poolKey.toId()]).initializePostLockerSetup(
                poolKey, locker[poolKey.toId()], karmaIsToken0[poolKey.toId()]
            );
    ...
    function _runMevModule(
        ...
        if (mevModuleOperational(poolKey.toId())) {
            ...
            bool disableMevModule = IKarmaMevModule(mevModule[poolKey.toId()]).beforeSwap(
                poolKey, swapParams, karmaIsToken0[poolKey.toId()], poolSwapData.mevModuleSwapData
            );
    ...
    function _runPoolExtensionHelper(
        ...
        IKarmaHookV2PoolExtension(poolExtension[poolKey.toId()]).afterSwap(
            poolKey, swapParams, delta, karmaIsToken0[poolKey.toId()], swapData
        );
    }
    ...
    function _lpLockerFeeClaim(PoolKey calldata poolKey) internal {
        ...
        // determine the token
        address token = karmaIsToken0[poolKey.toId()]
            ? Currency.unwrap(poolKey.currency0)
            : Currency.unwrap(poolKey.currency1);

        // trigger the fee claim
        IKarmaLpLocker(locker[poolKey.toId()]).collectRewardsWithoutUnlock(token);
    }

    function _hookFeeClaim(PoolKey calldata poolKey) internal {
        // determine the fee token
        Currency feeCurrency =
            karmaIsToken0[poolKey.toId()] ? poolKey.currency1 : poolKey.currency0;
    ...
    function _beforeSwap(
        ...
        bool token0IsKarma = karmaIsToken0[poolKey.toId()];
        bool swappingForKarma = swapParams.zeroForOne != token0IsKarma;
        bool isExactInput = swapParams.amountSpecified < 0;

        // case: specified amount paired in, unspecified amount karma out
        ...
        if (isExactInput && swappingForKarma) {
            ...
            // this also helps us match the ExactOutput swappingForKarma scenario
            ...
            poolManager.mint(
                address(this),
                token0IsKarma ? poolKey.currency1.toId() : poolKey.currency0.toId(),
                uint256(int256(fee))
            );
        }

        // case: specified amount paired out, unspecified amount karma in
        ...
        if (!isExactInput && !swappingForKarma) {
            ...
            // the ExactOutput !swappingForKarma scenario
            uint128 scaledProtocolFee = uint128(protocolFee) * 1e18 / (1_000_000 - protocolFee);
            int128 fee = int128(swapParams.amountSpecified * int128(scaledProtocolFee) / 1e18);
            delta = toBeforeSwapDelta(fee, 0);
    ...
    function _afterSwap(
        ...
        // case: specified amount karma in, unspecified amount paired out
        ...
        if (isExactInput && !swappingForKarma) {
            // grab non-karma amount out
            int128 amountOut = token0IsKarma ? delta.amount1() : delta.amount0();
            // take fee from it
        ...
        // case: specified amount karma out, unspecified amount paired in
        ...
        if (!isExactInput && swappingForKarma) {
            // grab non-karma amount in
            int128 amountIn = token0IsKarma ? delta.amount1() : delta.amount0();
        ...
        if (isExactInput && swappingForKarma || !isExactInput && !swappingForKarma) {
            if (karmaIsToken0[poolKey.toId()]) {
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IKarmaHook).interfaceId
            || interfaceId == type(IKarmaHookV2).interfaceId;
    }
