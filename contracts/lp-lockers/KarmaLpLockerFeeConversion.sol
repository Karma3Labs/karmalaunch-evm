...
import {IKarmaFeeLocker} from "../interfaces/IKarmaFeeLocker.sol";

import {IKarmaHook} from "../interfaces/IKarmaHook.sol";
import {IKarmaLpLocker} from "../interfaces/IKarmaLpLocker.sol";
import {IKarmaLpLockerFeeConversion} from "./interfaces/IKarmaLpLockerFeeConversion.sol";
...
contract KarmaLpLockerFeeConversion is IKarmaLpLockerFeeConversion, ReentrancyGuard, Ownable {
    ...
    IKarmaFeeLocker public immutable feeLocker;
    ...
    constructor(
        address owner_,
        address factory_, // Address of the karma factory
        address feeLocker_,
        ...
        factory = factory_;
        feeLocker = IKarmaFeeLocker(feeLocker_);
    ...
    function placeLiquidity(
        IKarma.LockerConfig memory lockerConfig,
        IKarma.PoolConfig memory poolConfig,
        ...
        FeeIn[] memory lpFeeConversionPreferences = abi.decode(
            lockerConfig.lockerData, (IKarmaLpLockerFeeConversion.LpFeeConversionInfo)
        ).feePreference;
    ...
    function _mintLiquidity(
        ...
        for (uint256 i = 0; i < lockerConfig.tickLower.length; i++) {
            ...
            if (lockerConfig.tickLower[i] < poolConfig.tickIfToken0IsKarma) {
        ...
        bool token0IsKarma = token < poolConfig.pairedToken;
        ...
        int24 startingTick =
            token0IsKarma ? poolConfig.tickIfToken0IsKarma : -poolConfig.tickIfToken0IsKarma;
        ...
        for (uint256 i = 0; i < lockerConfig.tickLower.length; i++) {
            ...
            uint256 amount0 = token0IsKarma ? tokenAmount : 0;
            uint256 amount1 = token0IsKarma ? 0 : tokenAmount;

            // determine tick bounds for this position
            int24 tickLower_ =
                token0IsKarma ? lockerConfig.tickLower[i] : -lockerConfig.tickLower[i];
            int24 tickUpper_ =
                token0IsKarma ? lockerConfig.tickUpper[i] : -lockerConfig.tickUpper[i];
            int24 tickLower = token0IsKarma ? tickLower_ : tickUpper_;
            int24 tickUpper = token0IsKarma ? tickUpper_ : tickLower_;
    ...
    function _mevModuleOperating(address token) internal view returns (bool) {
        ...
        if (!IKarmaHook(address(_tokenRewards[token].poolKey.hooks)).mevModuleEnabled(poolId)) {
            return false;
        }
        ...
        uint256 poolCreationTimestamp =
            IKarmaHook(address(_tokenRewards[token].poolKey.hooks)).poolCreationTimestamp(poolId);
        if (
            poolCreationTimestamp
                + IKarmaHook(address(_tokenRewards[token].poolKey.hooks)).MAX_MEV_MODULE_DELAY()
                <= block.timestamp
        ) {
            return false;
        }
        ...
        returns (uint256[] memory, uint256[] memory)
    {
        ...
        // if the rewardToken is the karma, we want to swap for recipients who
        // want their fee in the paired token
        //
        // conversely, if the rewardToken is the paired token, we want to swap for
        // recipients who want their fee in the karma token
        FeeIn toSwap = token == rewardToken ? FeeIn.Paired : FeeIn.Karma;
    ...
    function onERC721Received(address, address from, uint256 id, bytes calldata)
        ...
        returns (bytes4)
    {
        // Only Karma Factory can send NFTs here
        if (from != factory) {
            revert Unauthorized();
        }
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IERC721Receiver).interfaceId
            || interfaceId == type(IKarmaLpLocker).interfaceId;
    }
}