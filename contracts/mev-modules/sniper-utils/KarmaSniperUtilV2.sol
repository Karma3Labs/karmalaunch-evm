...
import {IKarmaSniperAuctionV0} from "../KarmaSniperAuctionV0.sol";

// shared util for snipers to bid in KarmaSniperAuction V0 and V2 auctions
contract KarmaSniperUtilV2 is ReentrancyGuard {
    ...
    IKarmaSniperAuctionV0 immutable karmaSniperAuction;
    IUniversalRouter immutable universalRouter;
    IPermit2 immutable permit2;
    address immutable weth;

    constructor(
        address _karmaSniperAuction,
        address _universalRouter,
        address _permit2,
        address _weth
    ) {
        karmaSniperAuction = IKarmaSniperAuctionV0(_karmaSniperAuction);
        ...
        returns (uint256 txGasPrice)
    {
        ...
        if (desiredBidAmount % karmaSniperAuction.paymentPerGasUnit() != 0) {
            revert InvalidBidAmount();
        }

        // calculate the gas price for the bid amount
        txGasPrice = auctionGasPeg + (desiredBidAmount / karmaSniperAuction.paymentPerGasUnit());
    }
        ...
        nonReentrant
    {
        ...
        if (round != karmaSniperAuction.round(poolId) || round > karmaSniperAuction.maxRounds())
        {
            revert InvalidRound();
        }

        // check that the correct block is being bid in
        if (block.number != karmaSniperAuction.nextAuctionBlock(poolId)) {
            revert InvalidBlock();
        }

        // check that the gas price is high enough
        int256 paymentUnits = int256(tx.gasprice) - int256(karmaSniperAuction.gasPeg(poolId));
        if (paymentUnits < 0) {
            revert GasPriceTooLow();
        }

        // check proper msg.value was sent in
        uint256 expectedBidAmount = karmaSniperAuction.paymentPerGasUnit() * uint256(paymentUnits);
        ...
        IERC20(weth).approve(address(karmaSniperAuction), msg.value);
        ...
        if (
            IKarmaHookV2(address(swapParams.poolKey.hooks)).supportsInterface(
                type(IKarmaHookV2).interfaceId
            )
        ) {
            // v2 hook
            swapParams.hookData = abi.encode(
                IKarmaHookV2.PoolSwapData({
        ...
        if (round + 1 != karmaSniperAuction.round(poolId)) {
