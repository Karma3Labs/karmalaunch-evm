...
import {IKarmaHookV2} from "../hooks/interfaces/IKarmaHookV2.sol";
import {IKarmaLpLocker} from "../interfaces/IKarmaLpLocker.sol";
import {IKarmaMevModule} from "../interfaces/IKarmaMevModule.sol";

import {IKarmaMevDescendingFees} from "./interfaces/IKarmaMevDescendingFees.sol";
import {IKarmaSniperAuctionV0} from "./interfaces/IKarmaSniperAuctionV0.sol";
...
contract KarmaSniperAuctionV2 is
    ReentrancyGuard,
    IKarmaSniperAuctionV0,
    IKarmaMevDescendingFees,
    Ownable
{
    ...
    IKarma public immutable karmaFactory;
    IKarmaFeeLocker public immutable feeLocker;

    constructor(address owner_, address _karmaFactory, address _feeLocker, address _weth)
        Ownable(owner_)
    {
        karmaFactory = IKarma(_karmaFactory);
        feeLocker = IKarmaFeeLocker(_feeLocker);
        weth = _weth;
        ...
        view
    {
        ...
        // ensure that the associated hook is a KarmaHookV2
        if (
            !IKarmaHookV2(address(poolKey.hooks)).supportsInterface(
                type(IKarmaHookV2).interfaceId
            )
        ) {
            revert OnlyKarmaHookV2();
        }

        // ensure the starting fee is not greater than the max mev LP fee
        if (feeConfigData.startingFee > IKarmaHookV2(address(poolKey.hooks)).MAX_MEV_LP_FEE()) {
        ...
        if (
            feeConfigData.secondsToDecay
                > IKarmaHookV2(address(poolKey.hooks)).MAX_MEV_MODULE_DELAY()
        ) {
        ...
        onlyHook(poolKey)
    {
        ...
        IKarmaMevDescendingFees.FeeConfig memory feeConfigData =
            abi.decode(descendingFeeConfig, (IKarmaMevDescendingFees.FeeConfig));
    ...
    function _sendPayment(PoolKey calldata poolKey, bool karmaIsToken0, uint256 paymentAmount)
        internal
    {
        if (paymentAmount == 0) {
            return;
        ...
        SafeERC20.safeTransfer(IERC20(weth), address(karmaFactory), factoryPayment);

        address karma = karmaIsToken0
            ? Currency.unwrap(poolKey.currency0)
            : Currency.unwrap(poolKey.currency1);

        // grab locker address from factory
        address lpLocker = karmaFactory.tokenDeploymentInfo(karma).locker;

        // get reward info from the locker
        IKarmaLpLocker.TokenRewardInfo memory tokenRewardInfo =
            IKarmaLpLocker(lpLocker).tokenRewards(karma);
    ...
    function _setLpFee(PoolKey calldata poolKey, uint24 lpFee) internal {
        ...
        IKarmaHookV2(msg.sender).mevModuleSetFee(poolKey, lpFee);
    }
    ...
    function beforeSwap(
        ...
        _sendPayment(poolKey, karmaIsToken0, paymentAmount);
