...
import {KarmaDeployer} from "./utils/KarmaDeployer.sol";
...
contract Karma is OwnerAdmins, ReentrancyGuard, IKarma {
    ...
    function setHook(address hook, bool enabled) external onlyOwnerOrAdmin {
        // check that the hook supports the IKarmaHook interface
        if (!IKarmaHook(hook).supportsInterface(type(IKarmaHook).interfaceId)) {
            revert InvalidHook();
        }
    ...
    function setLocker(address locker, address hook, bool enabled) external onlyOwnerOrAdmin {
        // check that the locker supports the IKarmaLpLocker interface
        if (!IKarmaLpLocker(locker).supportsInterface(type(IKarmaLpLocker).interfaceId)) {
    ...
    function setMevModule(address mevModule, bool enabled) external onlyOwnerOrAdmin {
        // check that the mev module supports the IKarmaMevModule interface
        if (!IKarmaMevModule(mevModule).supportsInterface(type(IKarmaMevModule).interfaceId)) {
    ...
    function setExtension(address extension, bool enabled) external onlyOwnerOrAdmin {
        // check that the extension contract supports the IKarmaExtension interface
        if (!IKarmaExtension(extension).supportsInterface(type(IKarmaExtension).interfaceId)) {
        ...
        returns (address tokenAddress)
    {
        ...
        tokenAddress = KarmaDeployer.deployToken(tokenConfig, TOKEN_SUPPLY);
    }
        ...
        returns (address tokenAddress)
    {
        ...
        // deploy the token
        tokenAddress = KarmaDeployer.deployToken(deploymentConfig.tokenConfig, TOKEN_SUPPLY);
        ...
        emit TokenCreated({
            ...
            poolId: poolKey.toId(),
            startingTick: deploymentConfig.poolConfig.tickIfToken0IsKarma,
        ...
        internal
    {
        ...
        // initialize the mev module
        IKarmaHook(deploymentConfig.poolConfig.hook).initializeMevModule(
    ...
    function _initializePool(
        ...
        poolKey = IKarmaHook(poolConfig.hook).initializePool(
            newToken,
            poolConfig.pairedToken,
            poolConfig.tickIfToken0IsKarma,
    ...
    function _initializeLiquidity(
        ...
        IKarmaLpLocker(lockerConfig.locker).placeLiquidity(
    ...
    function _triggerExtensions(
        ...
        for (uint256 i = 0; i < deploymentConfig.extensionConfigs.length; i++) {
            ...
            IKarmaExtension(deploymentConfig.extensionConfigs[i].extension).receiveTokens{
