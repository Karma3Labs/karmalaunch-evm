...
interface IKarmaPresaleEthToCreator is IKarmaExtension {
    ...
    event KarmaFeeRecipientUpdated(address oldRecipient, address recipient);
    event MinLockupDurationUpdated(uint256 oldMinLockupDuration, uint256 minLockupDuration);
    event KarmaDefaultFeeUpdated(uint256 oldKarmaDefaultFee, uint256 karmaDefaultFee);
    event KarmaFeeUpdatedForPresale(uint256 presaleId, uint256 oldKarmaFee, uint256 karmaFee);
    event SetAllowlist(address allowlist, bool enabled);

    struct Presale {
        PresaleStatus status; // current status of the presale
        IKarma.DeploymentConfig deploymentConfig; // token to be deployed upon successful presale
        ...
        uint256 karmaFee; // karma's fee to take in weth on successful presale
    }
