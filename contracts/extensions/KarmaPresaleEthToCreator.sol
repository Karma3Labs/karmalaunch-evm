...
import {IKarmaPresaleAllowlist} from "./interfaces/IKarmaPresaleAllowlist.sol";
import {IKarmaPresaleEthToCreator} from "./interfaces/IKarmaPresaleEthToCreator.sol";
...
contract KarmaPresaleEthToCreator is ReentrancyGuard, IKarmaPresaleEthToCreator, OwnerAdmins {
    ...
    constructor(address owner_, address factory_, address karmaFeeRecipient_)
        OwnerAdmins(owner_)
    {
    ...
    function setKarmaDefaultFee(uint256 karmaDefaultFeeBps_) external onlyOwner {
        if (karmaDefaultFeeBps_ >= BPS) revert InvalidKarmaFee();

        uint256 oldFee = karmaDefaultFeeBps;
        karmaDefaultFeeBps = karmaDefaultFeeBps_;

        emit KarmaDefaultFeeUpdated(oldFee, karmaDefaultFeeBps_);
    }

    function setKarmaFeeForPresale(uint256 presaleId, uint256 newFee)
        external
        presaleExists(presaleId)
        onlyOwner
    {
        // can only set lower
        if (newFee >= presaleState[presaleId].karmaFee) revert InvalidKarmaFee();

        uint256 oldFee = presaleState[presaleId].karmaFee;
        presaleState[presaleId].karmaFee = newFee;
        emit KarmaFeeUpdatedForPresale(presaleId, oldFee, newFee);
    }
    ...
    function setKarmaFeeRecipient(address recipient) external onlyOwner {
        address oldRecipient = karmaFeeRecipient;
        karmaFeeRecipient = recipient;
        emit KarmaFeeRecipientUpdated(oldRecipient, recipient);
    }
    ...
    function startPresale(
        ...
        if (allowlist != address(0)) {
            IKarmaPresaleAllowlist(allowlist).initialize(
        ...
        emit PresaleStarted({
            ...
            lockupDuration: lockupDuration,
            vestingDuration: vestingDuration,
            karmaFeeBps: karmaDefaultFeeBps
        });
    }
        ...
        nonReentrant
    {
        ...
        if (presale.allowlist != address(0)) {
            uint256 allowedAmount = IKarmaPresaleAllowlist(presale.allowlist)
    ...
    function claimEth(uint256 presaleId, address recipient) external presaleExists(presaleId) {
        ...
        // determine fee
        uint256 fee = (presale.ethRaised * presale.karmaFee) / BPS;
        ...
        // send eth to karma
        if (fee > 0) {
            (bool sent,) = payable(karmaFeeRecipient).call{value: fee}("");
            if (!sent) revert EthTransferFailed();
        }
    ...
    function receiveTokens(
        ...
        if (deploymentConfig.extensionConfigs[extensionIndex].msgValue != 0 || msg.value != 0) {
            revert IKarmaExtension.InvalidMsgValue();
        }
